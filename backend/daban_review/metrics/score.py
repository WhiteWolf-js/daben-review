...
- 因子阈值对标公开实证 + 本地隔日溢价回测校准:封流比对溢价严格单调(本地回测,最强因子)、
  封板越早溢价越高(国金研报,仅 ≤9:45 兑现)、换手越低越好(本地回测 + 华安研报「缩量锁筹」,
...
def grade_candidate(stock: dict, phase_hint: str) -> dict:
    ...
    # 首封时间:回测显示只有 ≤9:45 有超额(+2.3% vs 全档 ~1%),9:45 后各档无区分 → 只留超早封与午后罚
    if fmin is not None:
        if fmin <= 9 * 60 + 45:
            score += 2
            reasons.append("超早封(≤9:45,隔日溢价最高)")
        elif fmin >= 13 * 60:
            score -= 1
            reasons.append("晚封(午后,溢价弱)")
        # 9:45–13:00 中性,回测无区分
    ...
    if grade == "A+":
        hard_ok = (
            fmin is not None and fmin <= 9 * 60 + 45  # 超早封 ≤9:45

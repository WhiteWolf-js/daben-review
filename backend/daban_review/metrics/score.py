...
# 情绪周期 →(基准分, 仓位建议)。
#
# **基准分按本地回测的实测赔率排,不照抄「冰点空仓/退潮空仓」那套口号**。
# 1999 样本 / 28 交易日,隔日开盘溢价口径(胜率 / 均溢价 / 逐日为正的天数):
#   修复 69% / +2.20% / 6-6   发酵 73% / +2.43% / 1-1   高潮 57% / +1.56% / 5-5
#   退潮 53% / +1.16% / 8-8   分歧 50% / +0.81% / 7-8
# 退潮实测排中游而不是垫底,分歧才是最差的 —— 故两者同为 −1,差别放在仓位上。
# **仓位与基准分是两件事**:溢价口径只衡量「次日开盘能不能溢价走掉」,量不到退潮期
# 连续性差、拿不住的风险,所以退潮仓位仍压到 ≤1成。
_PHASE_BASE: dict[str, tuple[int, str]] = {
    "高潮": (0, "2-3成"),   # 全民狂热,追高风险大,减仓落袋
    "发酵": (1, "4-6成"),   # 主升加仓做主线
    "修复": (0, "3-4成"),   # 情绪试探性修复
    "分歧": (-1, "2-3成"),  # 实测最差的周期:中位溢价普遍贴 0,原来给 0 分给高了
    "冰点": (-1, "≤2成"),   # 观望等拐点(这 28 天零样本,沿用保守值)
    "退潮": (-1, "≤1成"),   # 仓位压住,但评级不再额外降级(见文件末尾注释)
    "未知": (0, "3成"),
}

# (原 _DOWNGRADE 表随「退潮全线降级」一起删除 —— 没有别的调用方)
...
def grade_candidate(stock: dict, phase_hint: str) -> dict:
    ...
    # 退潮期原本在 base 之外再全线降 1 级(照抄市面共识「退潮所有推荐降 1-2 级」),
    # **已按本地回测删掉** —— 1999 样本 / 28 交易日实测:退潮 n=349 胜率 53% 均溢价 +1.16%,
    # 逐日 8 天全为正(+0.2~+2.0),比分歧(50% / +0.81% / 8 天里 1 天亏)还好一档。
    # 「base −2 再降一级」把实测中游的周期罚成了最差,和「因子阈值以自己的回测为准」同一个教训。
    # 退潮的真实风险是连续性差、拿不住,那个由仓位(≤1成)控制,不该重复罚在评级上。
